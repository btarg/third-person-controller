...
func take_damage(damage: float, damage_type: BattleEnums.EAffinityElement = BattleEnums.EAffinityElement.PHYS) -> void:
    ...
    if affinities.has(damage_type):
        ...
        if affinity_type == BattleEnums.EAffinityType.WEAK:
            print(character_name + " is weak to " + str(damage_type))
            damage *= 2.0
        elif affinity_type == BattleEnums.EAffinityType.RESIST:
            print(character_name + " resists " + str(damage_type))
            damage *= 0.5
