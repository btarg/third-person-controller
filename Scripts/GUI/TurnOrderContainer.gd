...
func remove_entry(entry_character: BattleCharacter) -> void:
    ...
    for entry in entries:
        if character_by_entry.get(entry) == entry_character:
            ...
            entry.OnClicked.disconnect(_on_entry_focus_entered.bind(entry))
...
func add_entry(entry_character: BattleCharacter) -> void:
    ...
    entry_instance.OnClicked.connect(_on_entry_focus_entered.bind(entry_instance))

    entry_instance.get_node("Label").text = entry_character.character_name

    entry_instance.focus_entered.connect(_on_entry_focus_entered.bind(entry_instance))
...
# have a button highlighted, even if the mouse is away from the entries
func _on_entry_focus_entered(entry: Control) -> void:
