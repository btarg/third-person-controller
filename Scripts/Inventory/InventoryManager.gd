...
@onready var new_item := preload("res://Scripts/Inventory/Resources/new_item2.tres") as BaseInventoryItem

func _ready() -> void:
	inventory_updated.connect(_on_inventory_updated)
	add_item(new_item, 99)
...
func print_inventory() -> void:
	for item_id: String in items.keys():
		print("%s (%s): %s" % [items[item_id]["resource"].item_name, item_id, items[item_id]["count"]])
